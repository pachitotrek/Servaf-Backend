...
let DownloadPipe = class DownloadPipe {
    transform(img, tipo = 'usuario') {
        let url = _services_global__WEBPACK_IMPORTED_MODULE_2__["Global"].url + 'down';
        if (!img) {
...
let ImagenPipe = class ImagenPipe {
    transform(img, tipo = 'usuario') {
        let url = _services_global__WEBPACK_IMPORTED_MODULE_2__["Global"].url + 'img';
        if (!img) {
            return url + '/usuarios/xxx';
        }
        if (img.indexOf('https') >= 0) {

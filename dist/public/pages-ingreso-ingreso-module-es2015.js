...
let InformeComponent = class InformeComponent {
    ...
    getOrden(id) {
        this.api.certificadoOrden(id).subscribe((resp) => {
            if (resp.ok) {
    ...
    getMedidores(id) {
        this.api.certificadoOrdenMedidores(id).subscribe((resp) => {
            if (resp.ok) {
...
InformeComponent.ctorParameters = () => [
...
let IngresoPageRoutingModule = class IngresoPageRoutingModule {
};
...
let IngresoPageModule = class IngresoPageModule {
};
...
let IngresoPage = class IngresoPage {
    constructor() { }
    ngOnInit() {
    }
};
...
let InicioComponent = class InicioComponent {
    ...
    ngOnInit() { }
    get() {
        this.api.getingresos(this.form.value).subscribe((resp) => {
            if (resp.ok) {
...
let NuevoComponent = class NuevoComponent {
    constructor(eventos, api, modalController, aux, lc) {
        ...
        this.usuario = [];
        this.fecha = "";
        this.subscripcion = eventos.getNewMedidor().subscribe((x) => {
            this.getMedidores();
        });
        this.incidencia$ = eventos.getNewClient().subscribe((x) => {
            this.getClientes();
        });
    ...
    getMedidores() {
        this.api.getMedidoresOpen().subscribe((resp) => {
            if (resp.ok) {
                this.medidores = resp.data;
            }
        });
    ...
    close() {
        this.aux.createLoading().then((x) => {
            ...
            this.api.cerrarIngreso(a).subscribe((resp) => {
                if (resp.ok) {
    ...
    delete(id) {
        this.aux.createLoading().then((x) => {
            this.api.DeleteMedidor(id).subscribe((resp) => {
                if (resp.ok) {
                    this.aux.dismissLoading().then((b) => {
...
//# sourceMappingURL=pages-ingreso-ingreso-module-es2015.js.map